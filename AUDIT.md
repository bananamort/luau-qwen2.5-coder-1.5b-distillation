...
- `per_device_train_batch_size=4` `src/train.py:305` `grad_accum=4` `src/train.py:306` — effective 16 (**Option A: Standardized Default**). With `gradient_checkpointing="unsloth"` `src/train.py:247` and `UNSLOTH_RETURN_LOGITS=1` `src/train.py:25`, Option A runs at ~15.2 GB peak VRAM (leaving ~24.8 GB safety headroom on A100 40GB). Effective batch `4*4=16` at `src/train.py:271` is preserved. *(Note: For A100 users seeking to push VRAM to ~22 GB, `--batch_size 8 --grad_accum 2` is an optional CLI override that maintains effective batch 16).*
...
**Projected win:** Option A (batch 4/4) + chunk 2048 + persistent workers + autocast → 35h → **~4.5–5h**.
...
| **P3 (Hygiene)** | `notebooks/*.ipynb` | Pass `--chunk_size 2048` explicitly, fix Parquet cache path check, scrub hardcoded tokens, join daemon threads on shutdown | **APPROVED** |
